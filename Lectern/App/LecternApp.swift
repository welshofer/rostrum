...
struct LecternApp: App {
    ...
    @CommandsBuilder private var lecternCommands: some Commands {
        CommandGroup(replacing: .newItem) {
            Button("New Deck") { app.startCreate() }
                .keyboardShortcut("n")
            ...
            // The app's other errand, reachable without going Home first.
            // Export lives inside the result of this, not beside it: you
            // cannot sensibly export a deck you have not opened.
            Button("Inspect Deck…") { app.chooseDeckToInspect() }
                .keyboardShortcut("i")
                .disabled(app.phase == .generating || app.phase == .inspecting)
